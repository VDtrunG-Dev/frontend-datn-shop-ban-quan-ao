...
window.ChatLieuController = function($scope, $http, $location,$routeParams){
    let url = "http://localhost:8080/api/material";
...
$http
.get("http://localhost:8080/api/material/stopworking")
.then(function (response) {
  $scope.materialStops = response.data;
});
//restore category
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    $http
      .put("http://localhost:8080/api/material/restore/" + id)
      .then(function (response) {
...
$scope.pagerStop = {
    ...
    get items() {
      ...
      return $scope.materialStops.slice(start, start + this.size);
    },
    get count() {
      return Math.ceil((1.0 * $scope.materialStops.length) / this.size);
    },
  ...
  $scope.delete = function (id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http
          .delete("http://localhost:8080/api/material/delete/" + id)
          .then(function (response) {
            if (response.status === 200) {
    ...
    $scope.add = function(){
      ...
      $scope.update = function(){
        ...
        $http.put("http://localhost:8080/api/material/update/"+id,{
            name : $scope.form.name,
            description : $scope.form.description
        }).then(function(resp){
            if(resp.status === 200){
    ...
    //         /* Read more about isConfirmed, isDenied below */
    //         if (result.isConfirmed) {
    //             $http.put("http://localhost:8080/api/material/delete/"+id).then(function (response){
    //                 if (response.status === 200){
    //                     Swal.fire('Xóa thành công !', '', 'success')
    //                     $scope.loadAll();
    //                 }
    ...
    $scope.detail = function(){
        let id = $routeParams.id ;
        $http.get("http://localhost:8080/api/material/" + id).then(function(resp){
    ...
    $scope.search = function (){
        ...
        if (name.trim().length === 0){
            ...
            $http.get("http://localhost:8080/api/material/search/"+name).then(function (search){
