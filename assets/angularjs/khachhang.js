window.KhachHangController = function ($scope, $http, $location, $routeParams) {
    ...
    $http
      .get("http://localhost:8080/api/customer/stopworking")
      .then(function (response) {
        $scope.customerStops = response.data;
      });
    //restore category
    $scope.restore = function (id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $http
            .put("http://localhost:8080/api/customer/restore/" + id)
            .then(function (response) {
        ...
        .then(function (upImage) {
          $http
            ...
            .then(function (resp) {
              if (resp.status === 200) {
                $http
                  .post("http://localhost:8080/api/cart/addCart", {
    ...
    $scope.update = function () {
      ...
      $http
        .put("http://localhost:8080/api/customer/update/" + id, {
          code: $scope.form.code,
          fullname: $scope.form.fullname,
          username: $scope.form.username,
          password: $scope.form.password,
    ...
    //     }).then((result) => {
    //         if (result.isConfirmed) {
    //             $http.put("http://localhost:8080/api/customer/deletefake/" + id).then(function (response) {
    //                 if (response.status === 200) {
    ...
    $scope.pagerStop = {
      ...
      get items() {
        ...
        return $scope.customerStops.slice(start, start + this.size);
      },
      get count() {
        return Math.ceil((1.0 * $scope.customerStops.length) / this.size);
      },
    ...
    $scope.search = function () {
      ...
      if (fullname.trim().length === 0) {
        ...
        $http
          .get("http://localhost:8080/api/customer/search/" + fullname)
