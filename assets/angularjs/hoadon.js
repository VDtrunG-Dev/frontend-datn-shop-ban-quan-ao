window.HoaDonController = function (
  $scope,
  $http,
  $location,
  $routeParams,
  ...
  $http
    .get("http://localhost:8080/api/bill/getbystatus/0")
    .then(function (rest) {
      $scope.choxacnhan = rest.data.length;
    });
  $http
    .get("http://localhost:8080/api/bill/getbystatus/1")
    .then(function (rest) {
      $scope.chogiaohang = rest.data.length;
    });
  $http
    .get("http://localhost:8080/api/bill/getbystatus/2")
    .then(function (rest) {
      $scope.danggiaohang = rest.data.length;
    });
  $http
    .get("http://localhost:8080/api/bill/getbystatus/3")
    .then(function (rest) {
      $scope.dagiaohang = rest.data.length;
    });
  $http
    .get("http://localhost:8080/api/bill/getbystatus/4")
    .then(function (rest) {
      $scope.dahuy = rest.data.length;
    });
  $http
    .get("http://localhost:8080/api/bill/getbystatus/5")
    .then(function (rest) {
  ...
  $scope.loc = function (status) {
    ...
    if (status === null) {
      ...
      $http
        .get("http://localhost:8080/api/bill/getbystatus/" + status)
        .then(function (rest) {
          $scope.list = rest.data;
        });
    }
  ...
  $scope.getBill = function () {
    $http
      ...
      .then(function (resp) {
        ...
        $http
          .get("http://localhost:8080/api/billhistory/" + code)
          .then(function (resp1) {
  ...
  $scope.check = function (code, id) {
    ...
    if ($scope.bill.status === 2) {
      if ($scope.bill.payStatus === 0) {
        Swal.fire(
          "Vui lòng xác nhận thanh toán trước khi xác nhận đơn hoàn thành !",
          "",
          "error"
        );
        return;
              ...
              Swal.fire({
                ...
                if (result.value) {
                  ...
                  $http({
                    ...
                    $http
                      .get(
                        "http://localhost:8080/api/bill/getallbybill/" + code
                      )
                      .then(function (bill) {
                          ...
                          if (
                            ...
                            $http
                              ...
                              .then(function (billdetail) {
                                $http
                                  .get(
                                    "http://localhost:8080/api/bill/getbycode/" +
                                      code
                                  )
                                  .then(function (update) {
                                    var total =
                                      parseFloat(update.data.totalPrice) +
                                      parseFloat(pro.data.price * result.value);
                                    $http.put(
                                      "http://localhost:8080/api/bill/updateTongTien?Code=" +
                                        code +
                                        "&Money=" +
                                        total
                                    );
                                  ...
                                  $http.get(urlcolor).then(function (response) {
                                    ...
                                    $http
                                      ...
                                      .then(function (response) {
                                        ...
                                        let note =
                                          "Đã thêm sản phẩm " +
                                          bill.data[i].productDetail.product
                                            .name +
                                          " [ màu " +
                                          tenMau +
                                          ", kích thước " +
                                          tenKT +
                                          ", số lượng : " +
                                          bill.data[i].quantity +
                                          " ]";
                                        $http
                                          .post(
                                            "http://localhost:8080/api/billhistory",
                                            {
                                              createBy:
                            ...
                            $http({
                              ...
                              $http.get(urlcolor).then(function (response) {
                                for (let j = 0; j < response.data.length; j++) {
                                  if (
                                    response.data[j].id === resp.data.idColor
                                  ) {
                                    tenMau = response.data[j].name;
                                  }
                                }
                                ...
                                $http.get(urlsize).then(function (response) {
                                  for (
                                    ...
                                    if (
                                      response.data[j].id === resp.data.idSize
                                    ) {
                                      tenKT = response.data[j].name;
                                    }
                                  }
      ...
      if (result.isConfirmed) {
        ...
        var dataArray = $scope.list.map(function (item) {
          return {
            ...
            HinhThucThanhToan:
              item.payType == 1 ? "Đã thanh toán" : "Chưa thanh toán",
            TrangThaiThanhToan:
              item.payStatus == 0 ? "Chưa thanh toán" : "Đã thanh toán",
            TrangThaiDonHang:
              item.status == 0
                ? "Chờ xác nhận"
                : item.status == 1
                ...
                : "",
            HinhThucMuaHang:
              item.typeStatus == 1 ? "Mua tại quầy" : "Mua online",
          };
        });
  ...
  $scope.doihang = function (id) {
    ...
    $http
      .get(
        "http://localhost:8080/api/productdetail_color_size/getbyproduct/" + id
      )
      .then(function (response) {
                ...
                $http({
                  ...
                  $http.get(urlcolor).then(function (response) {
                    ...
                    $http.get(urlsize).then(function (response) {
                      ...
                      let note =
                        "Đã xóa sản phẩm " +
                        resp.data.productDetail.product.name +
                        " [ màu " +
                        tenMau +
                        ", kích thước " +
                        tenKT +
                      ...
                      $http
                        ...
                        .then(function (update) {
                          var total =
                            parseFloat(update.data.totalPrice) -
                            parseFloat(
                              resp.data.unitPrice * resp.data.quantity
                            );
                          $http.put(
                            "http://localhost:8080/api/bill/updateTongTien?Code=" +
                              code +
                      ...
                      $http({
                        ...
                        $http
                          .get(
                            "http://localhost:8080/api/bill/deleteBillDetail/" +
                              id
                          )
                          .then(function (resp) {
                            $http
  ...
  $scope.XacNhanTT = function () {
    ...
    $http
      ...
      .then(function (resp) {
        let amout =
          resp.data.totalPrice + resp.data.shipPrice - resp.data.totalPriceLast;
        Swal.fire({
          title:
          ...
          if (result.isConfirmed) {
            $http
              ...
              .then(function (re) {
                $http
                  .put("http://localhost:8080/api/bill/updateStatus/" + code, {
                    payStatus: 1,
  ...
  $scope.sendBillToMail = function (code) {
    $http
      .get("http://localhost:8080/api/bill/getbycode/" + code)
      .then(function (billexport) {
        $scope.billexport = billexport.data;
        $http
          .get(
            "http://localhost:8080/api/address/get/" + billexport.data.idAddress
          )
          .then(function (add) {
          ...
          .then(function (resp) {
            $http
              ...
              .then(function (resp) {
                ...
                $http
                  .get("http://localhost:8080/api/customer/" + IdCustomer)
                  .then(function (response) {
                    // Lấy phần tử bằng ID
                    ...
                    var emailData = {
                      ...
                      subject:
                        "Cảm ơn bạn đã mua hàng tại Dirtycoin lúc " +
                        hours +
                        " giờ " +
                        minutes +
                        " phút " +
                    ...
                    $http
                      .post("http://localhost:8080/api/sendmail", emailData)
                      .then(function (resp) {
