window.KichThuocController = function ($scope, $http, $location, $routeParams) {
  ...
  $http
    .get("http://localhost:8080/api/size/stopworking")
    .then(function (response) {
      $scope.sizeStops = response.data;
    });
  ...
  $scope.delete = function (id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http
          .delete("http://localhost:8080/api/size/delete/" + id)
          .then(function (response) {
          ...
          .catch(function (err) {
            ...
            $http
              .put("http://localhost:8080/api/size/deletefake/" + id)
              .then(function (response) {
                Swal.fire(
                  "Không thể xóa kích thước! Đã chuyển ngừng hoạt động",
                  "",
                  "error"
                );
                $scope.loadAll();
  ...
  $scope.restore = function (id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http
          .put("http://localhost:8080/api/size/restore/" + id)
          .then(function (response) {
            if (response.status === 200) {
  ...
  // pagination for stopped sizes
  $scope.pagerStop = {
    page: 0,
    size: 5,
    get items() {
      var start = this.page * this.size;
      return $scope.sizeStops.slice(start, start + this.size);
    },
    get count() {
      return Math.ceil((1.0 * $scope.sizeStops.length) / this.size);
    },
    first() {
      this.page = 0;
    },
